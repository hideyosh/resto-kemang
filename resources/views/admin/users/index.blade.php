...
<div class="min-h-screen bg-gray-100">
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="mb-6 flex gap-2">
            <a href="{{ route('admin.users.create') }}" class="bg-green-500 hover:bg-green-600 text-white px-6 py-2 rounded font-bold transition">
                ➕ Tambah User Baru
            </a>
        </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-bold">Dibuat</th>
                ...
                <tbody>
                    @forelse ($users as $user)
                        <tr class="border-b hover:bg-gray-50">
                            <td class="px-6 py-3">{{ ($users->currentPage() - 1) * $users->perPage() + $loop->iteration }}</td>
                            <td class="px-6 py-3">{{ $user->name }}</td>
                            <td class="px-6 py-3">{{ $user->email }}</td>
                            <td class="px-6 py-3 text-sm">{{ $user->created_at->format('d-m-Y') }}</td>
                            <td class="px-6 py-3 text-center">
                                <a href="{{ route('admin.users.edit', $user->id) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm transition">
                                    Edit
                                </a>

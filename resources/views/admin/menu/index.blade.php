...
@section('content')
    <div class="min-h-screen bg-gray-100 text-black">
        ...
        <nav class="bg-black text-white px-6 py-4">
            <div class="max-w-7xl mx-auto flex justify-between items-center">
                ...
                <form method="POST" action="{{ route('logout') }}" class="inline">
                    @csrf
                    <button type="submit" class="bg-red-600 hover:bg-red-700 px-4 py-2 rounded transition text-white">
                        Logout
                    </button>
                </form>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 py-8 text-black">
            <div class="mb-4">
                <a href="{{ route('admin.dashboard') }}" class="text-blue-500 hover:text-blue-700 font-medium">
            ...
            @if (session('success'))
                <div class="bg-green-100 border border-green-400 text-black px-4 py-3 rounded mb-4">
                    {{ session('success') }}
                </div>
            @endif
                ...
                <table class="w-full text-black">
                    <thead class="bg-gray-200 text-black">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-sm font-bold text-black">Kategori</th>
                            <th class="px-6 py-3 text-left text-sm font-bold text-black">Harga</th>
                    ...
                    <tbody class="text-black">
                        @forelse ($menus as $menu)
                            <tr class="border-b hover:bg-gray-50 text-black">
                                ...
                                <td class="px-6 py-3 text-black">{{ $menu->name }}</td>
                                <td class="px-6 py-3 text-black">{{ ucfirst($menu->category) }}</td>
                                ...
                                <td class="px-6 py-3 text-center text-black">
                                    ...
                                    <form method="POST" action="{{ route('admin.menu.destroy', $menu->id) }}"
                                        class="inline" onsubmit="return confirm('Yakin ingin menghapus menu ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-3 text-center text-black">
